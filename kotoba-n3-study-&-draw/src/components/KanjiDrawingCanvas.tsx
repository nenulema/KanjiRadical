...
export default function KanjiDrawingCanvas({ 
  ...
  return (
    <div id="drawing-notebook-panel" className="bg-dark-panel rounded-2xl shadow-md border border-white/10 p-5 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fadeIn">
      {/* Canvas Drawing Left Side (7 Cols) */}
      <div className="lg:col-span-7 flex flex-col items-center">
        ...
        <div className="relative w-full max-w-[340px] md:max-w-[380px] aspect-square bg-[#121212] border-2 border-white/15 rounded-xl overflow-hidden shadow-inner group">
          ...
          >
            <span 
              style={{ fontSize: `${getFontSizePixel(kanji.length)}px` }}
              className="font-sans font-extrabold text-[#DFC15D] antialiased select-none tracking-normal"
            >
              {kanji}
            </span>
          </div>
      ...
      {/* Scoring, Evaluator & Feedback Right Side (5 Cols) */}
      <div className="lg:col-span-12 xl:col-span-5 flex flex-col justify-between bg-white/[0.01] rounded-xl p-5 border border-white/10">
        <div>
          <div className="flex items-center gap-2 mb-4">
            <Award className="text-brand-gold" size={18} />
            <h4 className="text-sm font-semibold text-[#E2E8F0]">Analisis Goresan</h4>
          </div>

          {/* Quick score card */}
          <div className="bg-white/[0.02] rounded-xl p-4 border border-white/10 shadow-md flex items-center justify-between mb-4">
            <div>
              <p className="text-xs text-white/40 font-mono uppercase tracking-wider">Akurasi Goresan Lokal</p>
              <p className="text-xl font-bold text-brand-gold mt-1 font-mono">
                {localScore !== null ? `${localScore}%` : "Mulai menggambar..."}
              </p>
            </div>
            <div className="flex flex-col items-end">
              {localScore !== null ? (
                localScore >= 70 ? (
                  <span className="px-2 py-0.5 text-xs font-bold bg-emerald-500/10 text-emerald-400 rounded-full border border-emerald-500/20 font-mono">
                    Sangat Bagus
                  </span>
                ) : localScore >= 40 ? (
                  <span className="px-2 py-0.5 text-xs font-bold bg-blue-500/10 text-blue-400 rounded-full border border-blue-500/20 font-mono">
                    Cukup Akurat
                  </span>
                ) : (
                  <span className="px-2 py-0.5 text-xs font-bold bg-white/5 text-white/40 rounded-full font-mono">
                    Kurang Akurat
                  </span>
                )
              ) : (
                <span className="text-xs text-white/30">Hubungkan template</span>
              )}
            </div>
          </div>

          {/* AI Evaluator Call-To-Action */}
          {!aiEvaluation && !isEvaluating && (
            <div className="bg-white/[0.02] rounded-xl p-4 border border-white/10 text-center flex flex-col items-center shadow-lg">
              <Sparkles className="text-brand-gold mb-2 animate-pulse" size={24} />
              <h5 className="text-xs font-bold text-brand-gold tracking-wide">Evaluasi AI Gemini Tutor</h5>
              <p className="text-[11px] text-white/50 mt-1 max-w-[240px] leading-relaxed mx-auto">
                Kirim goresan tangan Anda kepada AI untuk review kaligrafi mendalam, proporsi, keseimbangan, serta tips menulis.
              </p>
              <button
                onClick={checkDrawingWithAI}
                disabled={localScore === null || localScore < 10}
                className="mt-3.5 w-full bg-brand-gold hover:opacity-95 text-dark-bg font-extrabold text-xs py-2 px-4 rounded-lg shadow-md transition-all border border-brand-gold flex items-center justify-center gap-1.5 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
              >
                <Sparkles size={14} />
                Evaluasi AI Calligraphy
              </button>
              {localScore !== null && localScore < 10 && (
                <p className="text-[9px] text-red-400 mt-1.5 font-semibold">
                  Harap gambar kanji terlebih dahulu sebelum memulai evaluasi AI.
                </p>
              )}
            </div>
          )}

          {/* Loading state for Gemini evaluation */}
          {isEvaluating && (
            <div id="ai-evaluation-loading" className="flex flex-col items-center justify-center p-8 bg-dark-panel border border-white/10 rounded-xl shadow-sm text-center">
              <RefreshCw className="animate-spin text-brand-gold mb-3" size={24} />
              <p className="text-xs font-bold text-brand-gold">Menghubungkan ke Gemini AI...</p>
              <p className="text-[11px] text-white/50 mt-1">
                Tutor AI sedang mengevaluasi keseimbangan kuas dan urutan goresan Anda.
              </p>
            </div>
          )}
          ...
          {/* Error Message */}
          {evalError && (
            <div className="p-4 bg-red-500/10 text-red-400 border border-red-500/20 rounded-xl text-xs mt-3">
              <p className="font-extrabold">Evaluasi Gagal:</p>
              <p className="mt-1">{evalError}</p>
              <button 
                onClick={checkDrawingWithAI} 
                className="mt-2 text-brand-gold underline font-bold bg-transparent border-0 cursor-pointer block hover:opacity-80"
              >
                Coba Lagi
              </button>
            </div>
          ...
          {/* High quality AI appraisal layout (S, A, B, C grade dashboard) */}
          {aiEvaluation && (
            <div className="bg-dark-panel border border-white/10 rounded-xl overflow-hidden shadow-lg transition-all animate-fadeIn">
              {/* Score header band */}
              <div className="px-4 py-3 bg-white/[0.02] border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold uppercase tracking-wider text-brand-gold font-mono">Appraisal Grade</span>
                  <span className="flex items-center text-xs font-bold text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/30 font-mono">
                    <CheckCircle size={10} className="mr-0.5" /> Terverifikasi AI
                  </span>
                </div>
                <button
                  onClick={() => setAiEvaluation(null)}
                  className="text-xs text-brand-gold hover:underline cursor-pointer font-bold bg-transparent border-none"
                >
                  Ulangi
                </button>
              </div>
              ...
              <div className="p-4">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-tr from-brand-gold to-amber-600 flex items-center justify-center text-dark-bg font-black text-3xl shadow-md border-2 border-brand-gold">
                    {aiEvaluation.grade}
                  </div>
                  <div>
                    <h5 className="font-bold text-[#E2E8F0] text-sm">Hasil Review Kaligrafi</h5>
                    <div className="flex items-center gap-3 mt-1.5 font-mono text-[11px] text-white/50">
                      <div>Skor: <span className="font-bold text-[#E2E8F0] font-mono">{aiEvaluation.score}/100</span></div>
                      <div>Keseimbangan: <span className="font-bold text-[#E2E8F0] font-mono">{aiEvaluation.balanceScore}%</span></div>
                    </div>
                  </div>
                </div>

                {/* Indonesian Tutor feedback paragraphs */}
                <p className="text-xs text-[#E2E8F0] leading-relaxed bg-white/[0.02] p-3 rounded-lg border border-white/10 italic font-serif">
                ...
                {aiEvaluation.suggestions && aiEvaluation.suggestions.length > 0 && (
                  <div className="mt-4">
                    <h6 className="text-xs font-extrabold text-brand-gold mb-1.5 uppercase tracking-wide font-mono">Saran dari Sensei:</h6>
                    <ul className="space-y-1.5">
                      {aiEvaluation.suggestions.map((sug, idx) => (
                        <li key={idx} className="text-xs text-white/70 flex items-start gap-1.5">
                          <ChevronRight size={12} className="text-brand-gold mt-1 shrink-0" />
                          <span>{sug}</span>
                        </li>
                      ))}
                    </ul>
        ...
        {/* Action guidelines */}
        <div className="mt-6 border-t border-white/10 pt-4 text-[11px] text-white/30">
          <p className="flex items-center gap-1.5">
            <HelpCircle size={12} className="text-white/20" />
            <span>Goresan kursor/sentuhan dideteksi secara dinamis sesuai struktur Kanji {kanji}.</span>
          </p>
        </div>
      </div>
    </div>
